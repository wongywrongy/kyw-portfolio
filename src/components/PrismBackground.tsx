...
export default function PrismBackground() {
  ...
  // Memoized optimized configuration for better performance
  ...
  // Initialize prism only once
  useEffect(() => {
    const container = containerRef.current
    if (!container || rendererRef.current) return

    const {
      height, baseWidth, animationType, glow, offset, noise, transparent,
      scale, colorFrequency, hoverStrength, inertia, bloom, suspendWhenOffscreen,
      timeScale, targetFPS
    } = config
  ...
  }, [config, cleanup]) // Only run once on mount

  // Update hue shift when theme changes (without recreating the prism)
  useEffect(() => {
    if (programRef.current && theme !== previousThemeRef.current) {
      const HUE = theme === 'dark' ? 0 : 0.5
      programRef.current.uniforms.uHueShift.value = HUE
